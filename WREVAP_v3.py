...
def WREVAP(input_ws, input_name, input_ext):  
    #### Initialize file paths
    set_input_paths(input_ws, input_name, input_ext)
...
def set_input_paths(input_ws, input_name, input_ext='.csv'):
    paths.csv = '%s\\%s%s' % (input_ws, input_name, input_ext)
    paths.ini = '%s\\%s%s' % (input_ws, input_name, '.ini')
    paths.res = '%s\\%s%s' % (input_ws, input_name, '.RES')
    paths.tgw = '%s\\%s%s' % (input_ws, input_name, '.TGW')
    paths.sol = '%s\\%s%s' % (input_ws, input_name, '.SOL')
     
def get_value_from_range(
    v_type=float, prompt='  Please enter choice: ',
    v_min=float('-inf'), v_max=float('+inf')):
    if v_type not in [float, int]: v_type = float
    while True:
        try:
            value = v_type(raw_input(prompt))
            if v_min <= value <= v_max: return value
...
def get_int_from_list(v_list, v_default=None):
    while True:
        try:
            ...
            if value in v_list: return value
        except ValueError: pass
        except TypeError: pass
...
def get_parameters():
    ...
    if os.path.isfile(paths.ini):
        ini_exist_flag = True
        print ((
            '\n  The following WREVAP parameter INI file was found: %s'+
            '\n\n  Do you want to enter WREVAP parameters from this INI file '+
            '[Y/N] (default Y) ') % (paths.ini))
        while True:
    ...
    if ini_exist_flag and ini_read_flag:
        print '\n  Reading %s file' % os.path.basename(paths.ini)
    ...
    #### Enter parameters manually
        ...
        print ((
            ...
            '      0 - TD IS DEW POINT IN DEG.%s (default)\n'+
        ...
        if IP == 1: prompt = '  ENTER STATION ALTITUDE [m]: '
        elif IP == 0: prompt = '  ENTER AVERAGE ATMOSPHERIC PRESSURE AT STATION: [mb]'
        ...
        if LK <> 0:
            ...
            DA = get_value_from_range(
                float, '  ENTER AVERAGE DEPTH OF LAKE [m]: ', 1, float('+inf'))
            #### SALT - Total dissolved solids or salinity
            SALT = get_value_from_range(
                float, '  ENTER TOTAL DISSOLVED SOLIDS OR SALINITY [mg/L or PPM]: ',
                0, float('+inf'))
            PPN = 0.0
        #### ISUM
        print ((
        ...
        print ('\n  Do you want to save entered parameters in an INI file '+
               '[Y/N] (default Y): ')
        while True:
...
def read_data_file():
    ...
    print '\n  Reading %s file' % os.path.basename(paths.csv)
    ...
    for test_i, test_line in enumerate(dat_lines):
        if ('YEAR' in test_line and 'LENGTH' in test_line and
            ...
            header_line = test_i
            data_line = test_i+1
            print ("    Assuming line %d is the field names\n      Fields: %s" %
                   ((header_line+1), " ".join(dat_lines[header_line].split(','))))
    dat_header = dict([
...
def read_tgw_file():
    ...
    ##    print ("  NOTE: Copying data in SOL file to TGW file")
    ##    shutil.copy2(paths.sol, paths.tgw)
    try: tgw_f = open(paths.tgw, 'r')
    except:
        print (("  ERROR: The TGW file does not exist or "+
                "could not be opened\n         (%s)") % paths.tgw)
        print ("         If running LK==3, try running LK==2 first\n"+
               "         Then copy the data in the SOL file to the TGW file")
        print ("         See section 6.6 and 6.7 in NHRI Paper No. 24")
        raise SystemExit()
    tgw_data = [float(i) for i in tgw_f.readlines()]
    tgw_f.closed
    if len(tgw_data)<>13:
        print "  ERROR: There should be 13 values in the .TGW file"
        raise SystemExit()
...
if __name__ == '__main__':
    ...
    #### Get user input
    #### User can set data filename as first argument to script
    #### If no arguments are set, prompt user for data filename
    #### filename can be full path, just filename.ini (use current working dir.),
    ####   or just filename (try .csv)
    try:
        user_input = sys.argv[1]
    except IndexError:
        user_input = raw_input('  Enter input data filename or path (i.e. sample.csv): ')

    #### Check user_input
    if os.path.isfile(user_input) and os.path.dirname(user_input):
        print "TEST1"
        input_ws = os.path.dirname(user_input)
        input_name = os.path.splitext(os.path.basename(user_input))[0]
        input_ext = os.path.splitext(os.path.basename(user_input))[-1]
    elif os.path.isfile(user_input+'.csv') and os.path.dirname(user_input):
        print "TEST2"
        print ('\n  File extension not specified, assuming .CSV extension\n')
        input_ws = os.path.dirname(user_input+'.csv')
        input_name = os.path.basename(user_input+'.csv')
        input_ext = '.csv'
    elif os.path.isfile('%s\\%s' % (os.getcwd(), user_input)):
        print "TEST3"
        print (('\n  File path not specified, using current working '+
                'directory (%s)\n') % os.getcwd())
        input_ws = os.getcwd()
        input_name = os.path.splitext(user_input)[0]
        input_ext = os.path.splitext(user_input)[-1]
    elif os.path.isfile('%s\\%s.csv' % (os.getcwd(), user_input)):
        print "TEST4"
        print (('\n  File path not specified, using current working '+
                'directory (%s)') % os.getcwd())
        print ('  File extension not specified, assuming .CSV extension\n')
        input_ws = os.getcwd()
        input_name = user_input
        input_ext = '.csv'
    else:
        print "TEST5"
        input_ws, input_name, input_ext = None, None, None
        print ('\n  ERROR: The input data file (%s) was not found' % user_input)
        print ('  ERROR: Please check the path and try again')

    #### Run WREVAP
    print input_ws, input_name, input_ext
    if input_ws and input_name and input_ext:
        input_file = input_name+input_ext
        print '  Data Workspace:  %s' % input_ws
        print '  Data Filename:   %s' % input_file
        WREVAP(input_ws, input_name, input_ext)
