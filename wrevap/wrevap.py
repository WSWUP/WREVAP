...
def WREVAP(input_path, data_path):
    ...
    Args:
        ...
        data_path (str): file path to CSV data file
    """
    ...
    if param.LK in [2, 3]:
        ...
        # DO FINAL CALCULATION AND LIST INPUTS AND THE
        # RESULTS FOR EACH PERIOD.
...
def get_parameters():
    ...
    if ini_exist_flag and ini_read_flag:
        ...
        if LK == 0:
            ...
            PPN = 0.0
            RD = read_param('RD', 0, int, config)
            check_param_in_list(RD, RD_list, 'RD')
        ...
        if LK != 0:
            ...
            RD = get_int_from_list(RD_list, 0)
        ...
        if CHR in ['Y', '']:
            ...
            par_str = ((
                ...
                '#  1 - TABULATION OF AVERAGED MONTHLY TOTALS IS LISTED\n' +
                'ISUM = {7:<1d}\n\n').format(
                    SITE, PHID, P, PPN, RD, DA, SALT, LK, ISUM, IT, IS, IV, IP))
            par_f.write(par_str)
            par_f.close()
...
def initialize():
    ...
    if param.ISUM == 1:
        ...
        data.TPPTM = defaultdict(dict)
        data.TDVM = defaultdict(dict)
        for DATE in data.DATE:
            data.TETM[DATE.month][DATE.year] = []
            data.TETPM[DATE.month][DATE.year] = []
            data.TRTM[DATE.month][DATE.year] = []
            data.TPPTM[DATE.month][DATE.year] = []
            data.TDVM[DATE.month][DATE.year] = []
...
def compute_budget_2(i):
    ...
    if param.ISUM == 1:
        ...
        data.TPPTM[DATE.month][DATE.year].append(PPT)
        data.TDVM[DATE.month][DATE.year].append(DV)
...
def compute_available_heat():
    ...
    for I in range(param.NN):
        SLT = 0.13 * data.DV[I]
        if SLT > (0.96 + 0.013 * data.DV[I]):
            SLT = 0.96 + 0.013 * data.DV[I]
        if SLT < (0.039 * data.DV[I]):
            SLT = 0.039 * data.DV[I]
...
def print_output():
    ...
    if param.LK == 0:
        ...
        LINE2_end = '{0:>10s}{1:>10s}{2:>10s}{3:>10s}{4:>10s}{5:>10s}'.format(
            'RAD.', 'POTENT.', 'LAKE', 'GW(W/M*M)', 'PPT', 'DV')
        res_f.write('{}{}{}\n'.format(LINE2_start, LINE2_mid, LINE2_end))
    ...
    for i, dt_start in enumerate(data.DATE):
        ...
        if param.LK <= 1:
            ...
            DV = '{0:10.2f}'.format(data.DV[i])

            res_f.write('{}\n'.format(''.join([
                YEAR, MONTH, DAY, LENGTH, TDW, TW, SW, HADD,
                RTMM, ETPMM, ETMM, GW, PPT, DV])))
...
def print_monthly_averages():
    ...
    for month in range(1, 13):
        ...
        for year in TRTM[month].keys():
            ...
            TPPTM[month][year] = sum(TPPTM[month][year])
            TDVM[month][year] = sum(TDVM[month][year])
        ...
        TPPTM[month] = sum(TPPTM[month].values()) / len(TPPTM[month].values())
        TDVM[month] = sum(TDVM[month].values()) / len(TDVM[month].values())

    # Sum of monthly averages
    RTNYR = sum(TRTM.values())
    ETPNYR = sum(TETPM.values())
    ETNYR = sum(TETM.values())
    PPTYR = sum(TPPTM.values())
    DVYR = sum(TDVM.values()) / 12.
    ...
    if param.LK == 0:
        ...
        res_f.write('{0:30s}{1:>10s}{2:^20s}\n'.format(
            ' ', 'NET ', '  EVAPORATION'))
        res_f.write('{0:20s}{1:>10s}{2:>10s}{3:>10s}{4:>10s}{5:>10s}{6:>10s}\n\n'.format(
            ' ', 'MONTH', 'RAD.', 'POTENT.', 'LAKE', 'PPT', 'DV'))
    for i in range(1, 13):
        MONTH = '{}'.format(calendar.month_abbr[i].upper())
        res_f.write('{0:>30s}{1:10.1f}{2:10.1f}{3:10.1f}{4:10.1f}{5:10.1f}\n'.format(
            MONTH, TRTM[i], TETPM[i], TETM[i], TPPTM[i], TDVM[i]))
    res_f.write('{0:30s}{1:>10s}{1:>10s}{1:>10s}{1:>10s}{1:>10s}\n\n'.format(' ', '_' * 8))
    res_f.write(
        '{0:>30s}{1:>10.1f}{2:>10.1f}{3:>10.1f}{4:>10.1f}{5:>10.1f}\n'.format(
            'TOTAL OF MONTHLY AVERAGES', RTNYR, ETPNYR, ETNYR, PPTYR, DVYR))
    res_f.close()
    return True
