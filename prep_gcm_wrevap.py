...
def prep_gcm_wrevap(workspace):
    ...
    ppt_month_factor_dict['Lake_Tahoe'] = [
        0.683793809, 0.69643266, 0.691828784, 0.712677417, 0.743181881, 0.668170098,
        0.543929977, 0.604882144, 0.607345296, 0.675140118, 0.678983427, 0.779749794]
    #### Old factors
    ##ppt_month_factor_dict['Lake_Tahoe'] = [
    ##    0.702914491, 0.720976639, 0.720924194, 0.774378465, 0.850691236, 0.828274127,
    ##    0.872059365, 0.866350328, 0.758308161, 0.738646093, 0.708893856, 0.804691107]
    ...
    try:
        ...
        if not os.path.isdir(input_ws):
            logging.error(
                ('\nERROR: The input workspace {0} does '+
                 'not exist').format(csv_name))
            raise SystemExit
        if not os.path.isdir(output_ws): os.mkdir(output_ws)

        res_param_path = os.path.join(workspace, res_param_name)
        if not os.path.isfile(res_param_path):
            logging.error(
                ('\nERROR: The reservoir parameter file (\'{0}\')'+
        ...
        if not os.path.isfile(default_ini_path):
            logging.error(
                ('\nERROR: The default config file (\'{0}\')'+
        ...
        for output_res in ppt_month_factor_dict.keys():
            if output_res not in output_res_list:
                logging.error('The reservoir name {0} is invalid'.format(output_res))
                raise SystemExit()
        ...
        for input_res, output_res in sorted(zip(
            input_res_list, output_res_list)):
            logging.info('{0}'.format(output_res))
            ...
            try:
                ...
                continue
            logging.info('  GCM Name: {0}'.format(input_res))

            #### Daily output file names
            tdew_csv_name = '{0}_TDEW.csv'.format(input_res)
            rso_csv_name = '{0}_RSO.csv'.format(input_res)
            rs_csv_name = '{0}_RS.csv'.format(input_res)
            ...
            if output_res not in output_res_array:
                logging.info('    Reservoir does not have data in {0}'.format(
                    output_res))
                continue
            ...
            ko_month_array = np.array([
                float(res_param[ko_field+month_abbr.upper()][0]) for 
            ...
            with open(tmax_csv_path, 'r') as f:
                ...
                if header_str[-1] == ',': header_str = header_str[:-1]
            f.closed
            ...
            if not np.all(tmax_tmin_mask):
                logging.warning(
                    '    WARNING: TMIN > TMAX for {0} cells'.format(
                ...
                for gcm, date in zip(
                    ...
                    ##bad_data_f.write(','.join([output_res, gcm, str(date)])+'\n')
                    logging.debug('    {0} {1}'.format(gcm, date))
                    del date, gcm
            ...
            if output_res in rs_month_factor_dict.keys():
                for month_i, month in enumerate(range(1,13)):
                    logging.info('    Scaling Monthly Rs: {0} {1}'.format(
            ...
            if output_res in ppt_month_factor_dict.keys():
                for month_i, month in enumerate(range(1,13)):
                    logging.info('    Scaling Monthly PPT: {0} {1}'.format(
            ...
            for gcm_i, gcm_name in enumerate(gcm_name_list):
                output_csv_name = '{0}_{1}.csv'.format(
            ...
            output_ini_name = '{0}.ini'.format(output_res.lower())
            output_ini_path = os.path.join(res_output_ws, output_ini_name)
            if os.path.isfile(output_ini_path): os.remove(output_ini_path)
            ...
            for default_line in default_ini_f:
                if default_line.strip().startswith('SITE = '):
                    ...
                    if wrevap_temp == 'C': default_line = 'IT = 0\n'
                    elif wrevap_temp == 'F': default_line = 'IT = 1\n'
...
def calc_tr_rso(lat, P, ea, J, deltaT, tmax_tmin, b0=0.031, b1=0.201, b2=-0.185):
