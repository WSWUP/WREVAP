...
def calc_gcm_wrevap(workspace):
    ...
    wrevap_path = os.path.join(workspace, wrevap_name)
    # wrevap_path = os.path.join(wrevap_ws, wrevap_name)
    if not os.path.isfile(wrevap_path):
        logging.error("wrevap path\n", wrevap_path, "\ndoes not exist.")
        sys.exit()
    ...
    # These are approximated factors that are much smoother from month to month
    ...
    # # These were original factors from Moreo (average of 2 years)
    ...
    for res_name in res_name_list:
        ...
        if not os.path.isdir(input_folder_ws):
            logging.info('  Input data folder\n{}\n is not present'.format(
                input_folder_ws))
            continue
        logging.info('Processing {}'.format(res_name))
        ...
        if not os.path.isfile(ini_path):
            logging.info(
                '  Configureation file {} is not present'.format(ini_name))
            continue
        ...
        if os.path.isdir(output_folder_ws) and overwrite_flag:
            logging.info(
                '  Removing existing output data folder {}'.format(
                    output_folder_name))
            shutil.rmtree(output_folder_ws)
            # DEADBEEF - I needed code to pause here for a minute
        ...
        for data_name in os.listdir(input_folder_ws):
            ...
            """
            # following produces goofy output when data_name has more than one  "_".
            logging.info('  {}'.format(
                data_name.split('.')[0].split('_', 1)[1]))
            """
            logging.info('Processing data file {}'.format(data_name))
            ...
            # Run 1st pass of WREVAP

            os.system('python {0} -i {1} --data {2}'.format(
                wrevap_name, ini_option2_name, data_name))
            """
            # following does not work ???? unix vs pc or path issue?
    ...
    for res in evap_month_factor_dict.keys():
        if res not in res_name_list:
            logging.error('Reservoir name {} is invalid'.format(res_name))
            raise SystemExit()
    ...
    for res_name in res_name_list:
        ...
        # Check that output data files exist

        output_folder_ws = os.path.join(res_ws, output_folder_name)
        if not os.path.isdir(output_folder_ws):
            logging.info('  Output data folder {} is not present'.format(
                output_folder_name))
            continue
